...
- For FTP questions, inspect `ftp-history` and the activity's recorded FTP basis. Apply an accepted change only through `set-ftp --watts ... --effective-date ... --reason ...`; do not replace the profile scalar directly. A legacy baseline is not a historical test result. Review affected workout wattages and stale budgets separately; changing FTP does not authorize rescaling either. See [dated FTP](https://github.com/rjmarsan/gradient-ascent/blob/main/docs/training-load.md#change-ftp-without-rewriting-earlier-load).
